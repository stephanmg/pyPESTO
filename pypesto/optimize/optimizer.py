...
class ScipyOptimizer(Optimizer):
    ...
    def minimize(
        ...
        if self.is_least_squares():
            ...
            # Set cxallback for handling timelimit if necessary
            callback = None
            if self._maxtime_seconds is not None and np.isfinite(self._maxtime_seconds):
                start_time = time.time()
                
                def callback(*args, **kwargs):
                    elapsed_time = time.time() - start_time
                    if elapsed_time >= self._maxtime_seconds:
                        raise StopIteration(f"Maximum time {self._maxtime_seconds}s exceeded")
